    ...
    with open(file_path, encoding="utf-8") as f:
        ...
        for line in f:
            if line.startswith("-DOCSTART-") or line == "" or line == "\n":
                if word:
                    examples.append(
                        InputExample(guid="{}-{}".format(lang, guid_index), words=words, labels=labels, langs=langs)
                    )
                    guid_index += 1
                    words = []
                    labels = []
        ...
        if words:
            examples.append(
                InputExample(guid="%s-%d".format(lang, guid_index), words=words, labels=labels, langs=langs)
            )
    return examples
...
def convert_examples_to_features(
  ...
  """
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        features.append(
            InputFeatures(
                input_ids=input_ids, input_mask=input_mask, segment_ids=segment_ids, label_ids=label_ids, langs=langs
            )
        )
    return features
